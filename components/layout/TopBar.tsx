        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="flex items-center gap-1.5 pl-1 pr-2 py-1 rounded-xl hover:bg-slate-100 transition-colors group">
              {/* Avatar */}
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white font-bold text-xs shadow-sm shadow-blue-200">
                {user.full_name?.[0]?.toUpperCase() ?? "W"}
              </div>
              {/* Nama — desktop only */}
              <span className="hidden md:block text-sm font-medium text-slate-700 group-hover:text-slate-900 max-w-[100px] truncate">
                {user.full_name}
              </span>
              {/* Chevron */}
              <ChevronDown
              ...
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50">
                ...
                <div className="px-3 pt-2 pb-1">
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2.5 px-2 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                ...
                <div className="px-3 pt-1 pb-2">
                  ...
                  <button
                    ...
                    disabled={loggingOut}
                    className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    {loggingOut ? (
