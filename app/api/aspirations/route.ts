...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json<ApiResponse>({ success: true, data: { items: data, total: count } })
}
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json<ApiResponse>({ success: true, message: 'Aspirasi berhasil dikirim', data }, { status: 201 })
}
